...
Step 6 (similar-customers) runs independently — can be parallelized with steps 3–5.
...
Add a **Branch** step as the first step in the agent. Six branches, evaluated top-to-bottom.
...
## Branch 6 — Similar Customers

**Condition:** Message contains `STEP: similar-customers`
...
**Instructions:**
```
The message contains COMPANY, INDUSTRY, and PRODUCTS fields in companyNameForResearch.

Search Glean for Docusign customers similar to this company — same industry,
...
```
The message JSON contains:
  step: "similar-customers"
  companyNameForResearch: JSON with company name, industry, active products, and product signals

Using the internal search results and your knowledge, identify 5–10 similar Docusign
...
**Instructions:**
~~~
Format the Think output as a JSON code block. Include all similar customers found,
ordered by relevance (strongest match first). Each customer must have all fields
populated — do not leave useCases, personas, or benefits empty.

If fewer than 5 similar customers were found, return what you have.
If no similar customers were found, return an empty array.

Return ONLY this JSON structure in a code block:
...
## GAS Integration (not yet implemented)

To wire Branch 6 into the GAS pipeline, the following would be needed:

1. **Message builder** — `_buildSimilarCustomersMessage(companyName, industry, activeProducts, productSignals)`
   sends `STEP: similar-customers` with company context.

2. **Pipeline call** — `_postToGleanStep('similar-customers', msg)` in `triggerGleanReport()`.
   Can run in parallel with steps 3–5 since it has no dependencies on Think outputs.

3. **Result assembly** — add `similarCustomers` to the `gleanAnalysis` object.

4. **Section builder** — new `addSimilarCustomersSection(body, similarCustomers)` in DocGenerator.gs.
